...
### 问题描述
/email_task 任务管理中创建任务时，明明已选择发件账号和联系人组，点击"创建任务"仍弹出
"请填写完整任务信息并选择至少一个发件账号和联系人组"。

### 根因分析
1. **误导性校验提示（主因）**：`createTask()` 把任务名称、发件账号、联系人组、邮件主题、邮件正文
   5 个校验合并成一个 `if`，任何一个为空都弹同一条提示。提示文案只提"发件账号和联系人组"，
   实际是邮件主题/正文为空（典型场景：下拉框选了模板但没点"使用模板"，主题和正文为空），
   用户误以为账号/组的选择丢失。
2. **正文判空不严谨**：`taskBody` 是 contenteditable，空内容可能是 `<br>`，直接判 `innerHTML`
   会把空正文当成已填写。
3. **联系人组选择器分页截断**：`loadGroupsForTask()` 调 `/api/group/list` 未传分页参数，
   后端默认 page_size=20，超过 20 个组时部分组在选择器中不可见。

### 修复方案
- `templates/email_task.html` `createTask()`：校验拆分为逐项检查，各自给出具体提示
  （任务名称/发件账号/联系人组/邮件主题/邮件内容）；名称和主题增加 trim；
  正文去 HTML 标签和 `&nbsp;` 后判空。
- `loadGroupsForTask()`：请求加 `?page=1&page_size=1000`，选择器加载全量组。

### 影响模块
- `templates/email_task.html`（仅此一个文件，前端校验与组加载，无后端/表结构改动）

---

## 2026-07-16: 邮件任务ET20260716180355315913001初始化SMTP连接超时导致任务失败

### 问题描述
邮件任务"0716 德国"（2753个联系人）启动后立即失败，错误信息为"timed out"，一封邮件都没发出去(sent_count=0)。

### 根因分析
1. SMTP_SSL timeout设置为20秒，但实际连接+登录耗时约9.7秒，接近边界
2. 当天已有2个大任务完成（日本4114封+越南378封），SMTP服务器可能限速
3. 初始化阶段每个账号只尝试1次连接，无重试机制
4. 不跳过已达日限的账号，浪费时间连接无法发送的账号
5. 发送中连接错误时，所有账号无法重连就直接终止任务，没有timeout阈值切换机制

### 修复方案
1. SMTP timeout 20→60秒（connect_smtp + send_test_email 两处）
2. 初始化阶段每个账号重试3次（5s/15s/45s指数退避）
3. 初始化时跳过已达日限的账号(can_send_today检查)
4. 发送循环中增加consecutive_timeout_count，连续3次timeout后切换到下一个账号，不终止任务
5. 所有发送成功时重置consecutive_timeout_count=0

### 影响模块
- `Sills/email_sender.py`（仅此一个文件）
...
## 2026-07-09: 报价客户过滤只显示4月前数据(展示/过滤不一致)

### 问题描述
报价订单页面用客户过滤，只显示今年4月份之前的数据，4月后的报价查不到。

### 根本原因
**展示逻辑与过滤逻辑取的客户字段不一致**：
- 展示/SELECT：`COALESCE(o.cli_id, c.cli_id)` —— 优先报价自己的客户
- 过滤/WHERE：`q.cli_id = ?` —— 只查询价表的客户

4月后报价多为直接录入（无 quote_id），客户存在 `o.cli_id`，`q.cli_id` 为空 -> 被过滤掉。
4月前报价通过询价转来，客户在 `q.cli_id` -> 正常显示。

### 修复方案
过滤条件改为 `COALESCE(o.cli_id, q.cli_id) = ?`，与展示逻辑完全一致。

### ⚠️ 重复 bug 模式（CLAUDE.md：出现 2 次以上的 bug 要记录）
| 时间 | 现象 | 根因 |
|---|---|---|
| 2026-06-20 | 联系人导出国家为空 | 导出用 SQL COALESCE，列表用 JS \|\|，空值语义不一致 |
| **2026-07-09 (本次)** | 报价客户过滤漏4月后数据 | 展示用 COALESCE(o.cli_id,...)，过滤用 q.cli_id，取字段不一致 |

**教训**：同一字段在"列表展示"和"筛选过滤"必须用**完全相同的取值逻辑**（同 COALESCE/同回退顺序）。凡是有"主字段+回退字段"设计的，过滤条件必须照搬展示的表达式，禁止各写各的。这是高频 bug 源，后续涉及"过滤/导出/列表一致性"应直接复用展示的字段表达式。
