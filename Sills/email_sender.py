...
class EmailSenderWorker:
    ...
    def run(self):
        ...
        consecutive_timeout_count = 0  # 当前账号连续timeout计数
        ...
        try:
            ...
            # 连接SMTP：跳过已达日限的账号，对每个账号重试3次（指数退避）
            server = None
            init_errors = []
            connected = False
            ...
            for attempt_idx in range(len(self.accounts)):
                idx = (start_index + attempt_idx) % len(self.accounts)
                acc = self.accounts[idx]

                # 跳过已达日限的账号
                can_send, remaining = can_send_today(acc.get('account_id'), daily_limit_per_account)
                if not can_send:
                    print(f"[Worker] 跳过账号 {acc.get('email')} (已达日限 {daily_limit_per_account})")
                    continue

                # 对每个账号重试3次（指数退避: 5s, 15s, 45s）
                for retry in range(3):
                    try:
                        ...
                        print(f"[Worker] 使用账号 {idx + 1}: {acc.get('email')} (第{retry + 1}次尝试成功)")
                        connected = True
                        break
                    except Exception as e:
                        wait_time = 5 * (3 ** retry)  # 5s, 15s, 45s
                        init_errors.append(f"{acc.get('email')}(第{retry + 1}次): {e}")
                        print(f"[Worker] 账号 {acc.get('email')} 第{retry + 1}次连接失败: {e}，等待{wait_time}秒后重试...")
                        if retry < 2:
                            time.sleep(wait_time)
            ...
            for idx, contact in enumerate(self.contacts):
                ...
                try:
                    ...
                    consecutive_timeout_count = 0  # 发送成功，重置timeout计数
                    ...
                    if self._is_connection_error(e):
                        # 检测是否为timeout错误，增加连续timeout计数
                        is_timeout = 'timed out' in error_msg.lower() or 'timeout' in error_msg.lower()
                        if is_timeout:
                            consecutive_timeout_count += 1
                            print(f"[Worker] {mode_str} [{current_account_email}] timeout ({consecutive_timeout_count}/3) {email_addr}")
                        else:
                            consecutive_timeout_count = 0  # 非timeout连接错误，重置计数

                        # 连续3次timeout → 切换到下一个账号，不终止任务
                        if is_timeout and consecutive_timeout_count >= 3:
                            print(f"[Worker] {mode_str} 账号 {current_account_email} 连续{consecutive_timeout_count}次timeout，切换到下一个账号")
                            consecutive_timeout_count = 0  # 重置计数
                            ...
                            if self.switch_to_next_account():
                                next_account = self.get_current_account()
                                # 跳过已达日限的账号
                                can_send_next, _ = can_send_today(next_account.get('account_id'), daily_limit_per_account)
                                if can_send_next:
                                    try:
                                        server = self.connect_smtp(next_account)
                                        print(f"[Worker] 已切换到账号 {next_account.get('email')}，继续发送")
                                        # 当前邮件记为失败，继续下一个联系人
                                        ...
                                        print(f"[Worker] 切换到账号 {next_account.get('email')} 连接失败: {conn_e}")
                                        # 继续尝试其他账号
                                else:
                                    print(f"[Worker] 账号 {next_account.get('email')} 已达日限，继续切换...")
                            # 无法切换或连接失败，继续尝试重连
                            print(f"[Worker] 无法切换到可用账号，尝试重连当前账号...")
                        ...
                        if success:
                            ...
                            try:
                                ...
                                consecutive_timeout_count = 0  # 重连后发送成功，重置timeout计数
                    ...
                    if '550' in error_msg and 'DT:SPM' in error_msg:
                        ...
                        # 非550错误，重置550计数器
                        consecutive_550_count = 0
                        consecutive_timeout_count = 0  # 非timeout错误，重置timeout计数
